...
export default function AdminOdemeler() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/odeme/kur`).then(r => setKur(r.data.USD_TRY)).catch(() => {})
  }, [filtre])
  ...
  const toplamUsd = kur ? (toplamBasarili / kur) : null

  const tutarStr = (tutar) => {
    const tl = `${tutar.toLocaleString('tr-TR')} ₺`
    if (!kur) return tl
    const usd = (tutar / kur).toFixed(2)
    return { tl, usd }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-[#C8CDD4] rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-green-600">{liste.filter(o => o.durum === 'basarili').length}</p>
          <p className="text-xs text-gray-500 mt-0.5">Başarılı Ödemeler</p>
        </div>
        <div className="bg-white border border-[#C8CDD4] rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-yellow-600">{liste.filter(o => o.durum === 'bekliyor').length}</p>
          <p className="text-xs text-gray-500 mt-0.5">Bekleyen</p>
        </div>
        <div className="bg-white border border-[#C8CDD4] rounded-xl p-4 text-center">
          <p className="text-xl font-bold text-[#0052CC]">
            {toplamUsd !== null ? `$${toplamUsd.toFixed(2)}` : `${toplamBasarili.toLocaleString('tr-TR')} ₺`}
          </p>
          {toplamUsd !== null && (
            <p className="text-xs text-gray-400">{toplamBasarili.toLocaleString('tr-TR')} ₺</p>
          )}
          <p className="text-xs text-gray-500 mt-0.5">Toplam Tahsilat</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {kur && <span className="text-xs text-gray-400 self-center">Kur: 1$ = {kur} ₺</span>}
      </div>
                ...
                return (
                  <tr key={o.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs">
                      {usd ? `${o.tutar.toLocaleString('tr-TR')} ₺` : '—'}
                    </td>
            ...
            <form onSubmit={kaydet} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  {kur && yeniForm.tutar > 0 && (
                    <p className="text-xs text-gray-400 mt-1">≈ ${(parseFloat(yeniForm.tutar) / kur).toFixed(2)}</p>
                  )}
                </div>
                <div>
